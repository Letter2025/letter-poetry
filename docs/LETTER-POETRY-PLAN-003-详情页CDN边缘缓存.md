...
> 计划状态：✅ 全部完成 ｜ 创建日期：2026-08-07 ｜ 完成日期：2026-08-07 ｜ 目标：为详情页/API 加 CDN 边缘缓存，减少 D1 查询与延迟；不依赖 KV（免费写入 1000/天不够预热 4.7 万首）
...
|---|---|
| 2026-08-07 | 创建计划 |
| 2026-08-07 | 完成：worker 用 Cache API 缓存页面（非 RSC），详情页线上 CF-Cache-Status: HIT 验证通过；API 保持 no-store |
| 2026-08-07 | 关键实测：Cloudflare 默认不缓存 Worker 响应（仅 Cache-Control 头无效），需 Cache API 显式 put |