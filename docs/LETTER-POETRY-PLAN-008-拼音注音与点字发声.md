...
> 计划状态：✅ 全部完成 ｜ 创建日期：2026-08-07 ｜ 完成日期：2026-08-07 ｜ 目标：正文拼音注音（全文注音模式 + 悬停拼音）+ 点击发声，零数据库改动
...
|---|---|
| 2026-08-07 | 创建计划 |
| 2026-08-07 | 完成：AnnotatedText（ruby 注音模式 + title 悬停拼音 + 点击发声，繁简兼容）；接入详情页/蒙学；pinyin-pro 验证（1:1、标点原样、繁体读音如裏=lǐ 髮=fà）；线上验证「全文注音」按钮 + title 拼音 |
| 2026-08-07 | 备注：首查未生效是部署传播延迟（CI run 时序判断失误）；worker/CDN 缓存详情页 1 天，新功能可用 ?x= 参数绕过 |