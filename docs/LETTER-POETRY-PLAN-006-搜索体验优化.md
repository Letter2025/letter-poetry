...
> 计划状态：✅ 全部完成 ｜ 创建日期：2026-08-07 ｜ 完成日期：2026-08-07 ｜ 目标：搜索命中行高亮 + 相关性排序，提升检索体验
...
|---|---|
| 2026-08-07 | 创建计划 |
| 2026-08-07 | 完成：db.ts 排序 CASE + computeHit；/poems 与选集页 Highlight mark；线上验证：API hit（戍鼓→命中行）、排序（春晓→标题前缀第一）、mark 渲染正确 |
| 2026-08-07 | 备注：主域名 /api 曾命中 CDN 旧缓存导致误判新旧代码；mark 为客户端渲染，curl 看不到，需浏览器/React 渲染验证 |